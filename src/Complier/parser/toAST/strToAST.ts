...
import { type ASTNodeType } from '@src/Complier/types'
...
function strToAST (template: string) {
  ...
  let index = 0
  let rest = template
  const root = [{ children: [] }] as ASTNodeType[]
  ...
  if (root.length > 1) {
    throw new Error(`template tag [${root[root.length - 1].tag as string}] not matched`)
  }
