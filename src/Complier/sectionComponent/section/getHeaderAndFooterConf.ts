...
function getTextList(text: string) {
...
function formatContent(type: string, conf: CommonConfType) {
...
function getAreaConf(area: string, formatConf: CommonConfType) {
  area = area || 'all'
  const areaKeyMap = {
  ...
  areaKeyMap[area] && areaKeyMap[area].forEach((key: string) => {
...
function getConf(type: string, conf: CommonConfType) {
